...
const buildBaseName = (title?: string) => {
  ...
  const slug = (title ?? "Report").trim().replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40);
...
export default function ChatWindow({ sessionId, session, onSendMessage, onOpenSidebar }: ChatWindowProps) {
  ...
  const handleDownloadPdf = async (markdown: string, index: number) => {
    ...
    setDlIndex(index);
    try {
      const baseName = buildBaseName(session?.title);
          ...
          <div className="space-y-6 p-4 sm:p-6 lg:p-8">
            <AnimatePresence initial={false}>
              {session.messages.map((msg, idx) => {
                ...
                const isLatest = !isUser && idx === lastAiIdx;
                    ...
                    <div className={`flex max-w-[85%] flex-col gap-2 ${isUser ? "items-end" : "items-start"}`}>
                      ...
                      {isLatest && !isError && (
                        <motion.button
                          ...
                          onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--c-400)"; e.currentTarget.style.color = "var(--c-900)"; }}
                          onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--c-200)"; e.currentTarget.style.color = "var(--c-700)"; }}
                        >
                          {dlIndex === idx ? (
